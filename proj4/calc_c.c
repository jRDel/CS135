...
scanf("%d",&input);	
	...
	switch(input){ /*The switch statement here is what allows the program to read user input, then perform that option embedded within each case.*/
		...
		break;
		case 7: /*Toggle by default is 1, so the case here goes to the else automatically and decrements toggle to 0. Then, the case has another while and switch loop within it which can print integers instead of doubles.*/
		if(toggle<=0){
		...
		toggle--;
		int term1,term2;
			while(input!=8&&toggle!=1){ /*This while loop continues as long as the user does not exit, and the toggle does not equal 1. In integer mode, the toggle is defaulted to 0 by the else statement above.*/
			printf("Options:\n");
			printf("1 - addition\n");
			printf("2 - subtraction\n");
			printf("3 - multiplication\n");
			...
			switch(input){	
				...
				default:
				if(input!=8&&input!=7&&input!=6&&input!=5&&input!=4&&input!=3&&input!=2&&input!=1){
